  ...
  return (
    <>
      <div className="fixed top-4 left-4 z-50 md:hidden">
        <IconButton onClick={toggleMenu} variant="secondary" size="lg" iconSize={24}>
          <Package />
        </IconButton>
      </div>

      {isOpen && <div onClick={closeMenu} className="fixed inset-0 bg-black/40 z-40 md:hidden" />}
          ...
          <nav className="flex flex-col bg-(--icon-bg) rounded-3xl p-1 gap-1 mt-25">
            {menuItems.map((item, index) => (
              <Link key={index} href={item.href} onClick={closeMenu}>
                <IconButton active={pathname === item.href}>{item.icon}</IconButton>
              </Link>
            ))}
          </nav>
        </div>
